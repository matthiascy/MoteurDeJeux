...
bool HomelandApp::_init_main_scene()
{
  ...
  */
  ...
  sponza->transform()->setPosition({0, 0, -10}, ESpace::World);
...
void HomelandApp::_init_lights()
{
  ...
  auto* lightComp000 = m_engine->componentManager()->addComponent<DirectionalLight>("DirectionalLight", light000, Vec3{0.9f, 0.9f, 0.9f}, 1.0f);

  // Directional
  auto* light004 = m_main_scene->createGameObject("light000", "default");
  light004->transform()->setPosition({0, 0, 10}, ESpace::World);
  light004->transform()->rotate(Math::AxisY, 180, ESpace::World);
  //m_engine->componentManager()->addComponent<MeshRenderer>("m", light000, m_assets["Cube"]);
  auto* lightComp004 = m_engine->componentManager()->addComponent<DirectionalLight>("DirectionalLight", light004, Vec3{0.4f, 0.9f, 0.4f}, 1.0f);
